...
void myTimerEvent()
{
  ...
  float gg = g;
  Blynk.virtualWrite(V7, gg);
  //Blynk.virtualWrite(V7,(4*3.14*3.14 *0.38)/sq);
...
BLYNK_WRITE(V8)
{
  ...
  digitalWrite(dirPin, HIGH);
  //Makes 200 Pulses for making one full cycle rotation
  for(int i = 0; i < 1150 * (active_length-15.5)-(active_length*active_length)/10.5; i++){
    digitalWrite(stepPin,HIGH); 
    delayMicroseconds(500); 
    digitalWrite(stepPin,LOW); 
  }
...
BLYNK_WRITE(V6)
{ 
  prev_exp_flag = exp_flag;
  exp_flag = param.asInt();
}
*/
void Experiment()
{ 
